...
class CharterSelection extends EditorTreeMenu {
	...
	public function saveSong(creation:SongCreationData) {
		...
		// Save Files
		CoolUtil.safeSaveFile('$songFolder/meta.json', Json.stringify(creation.meta, Constants.JSON_PRETTY_PRINT));
	...
	public function saveChart(name:String, data:ChartData) {
		...
		// Save Files
		CoolUtil.safeSaveFile('$songFolder/charts/${name}.json', Json.stringify(data, Constants.JSON_PRETTY_PRINT));
